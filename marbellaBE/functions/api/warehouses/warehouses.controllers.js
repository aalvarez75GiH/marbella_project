...
const getCheapestShippingRate = async (ship_to, ship_from, packages) => {
  try {
    ...
    if (!rates.length) {
      const error = new Error("No shipping rates found");
    ...
    const formattedError = new Error("Error getting shipping rates");
    formattedError.statusCode = error.statusCode || 500;
    formattedError.details = error.response?.data || error.details || null;
