...
export default function Customers_QR_Scanner_View() {
  ...
  const { getCustomersOrdersByQRToken } = useContext(OrdersContext);
  ...
  const validateCustomersOrders = async (token) => {
  ...
  const handleBarCodeScanned = ({ data }) => {
    ...
    if (!data.startsWith("marbella://customer/")) {
      scanLockRef.current = true;
      setIsScanningEnabled(false);

      Alert.alert("Invalid QR", "This code is not a Marbella customer's QR.");
      resetScannerSoon();
      return;
    }

    const token = data.replace("marbella://customer/", "").trim();
      ...
      {isValidating && (
        ...
        >
          <Global_activity_indicator
            caption="Wait, we getting customer's orders "
            caption_width="65%"
          />
        </Container>
      )}
      ...
      {!isValidating && (
        <View style={{ flex: 1 }}>
          ...
          >
            ...
            <Text variant="dm_sans_bold_22">Scan Customer's QR code</Text>
